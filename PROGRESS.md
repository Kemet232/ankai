...
| Real "Liquid Y2K" visual design applied (shared shell + Profile pane) | done — the validated glass-panel design system (`docs/design/tokens.md`, proven in `client/ui/spike-glass-blur.slint`, gated on two ADR-0002 spikes that closed back in session 3 but were never actually applied to the real app until now) is live on the sidebar/window chrome and the Profile pane: glass-tinted panels, glowing active-nav pill, avatar-initial header card, de-emphasized ID chips, and the Top 8 list rebuilt as real accent-cycled glass module cards with custom icon buttons instead of generic OS buttons. Messages/Hangouts got the same `FixedPanel` treatment in session 15; **Communities and Settings caught up in session 23** — see the dedicated row below. Every nav pane now has real glass content, not just glass chrome | `client/ui/app.slint` |
| Communities/Settings glass visual pass | **done (session 23)** — Communities split into "Create a Community"/"Your Communities" `FixedPanel`s with accent-cycled glass module-card rows (replacing plain `"• " + name` text), plus a "Post to `<community>`"/"Discussion" panel pair for forum posts; Settings split into "Profile"/"Directory Identity"/"Last.fm" `FixedPanel`s. Pure visual/structural pass — every callback preserved exactly, only `Theme.*` tokens used, both panes now scroll | `client/ui/app.slint` |
| Live-testing bug-fix round (session 23) | **done** — the human ran the freshly built app and reported five real, reproducible issues, each found by reading the actual code (no live-desktop clicking/screenshotting per this repo's standing caution) and fixed: (1) **background playback never stopped on nav-away** — `PlayerOverlay` and its 16ms frame-pump timer were gated only on `player-active`/`player-bounded-mode`, never on which nav tab was selected, so switching to Home while a video played left mpv decoding+compositing a hidden fullscreen frame behind that page indefinitely — a real ghost frame (with subtitle burn-in) and continuous CPU/GPU cost that read as app-wide stutter; a `changed selected-index` hook now closes the player for real on navigating away from Watch/Hangouts, and re-clicking the Discover nav item while already there (previously the one nav click with no effect, since `selected-index` doesn't change) also closes it, since Discover doubles as the now-playing surface and auto-hidden controls could otherwise strand a user with no way back to browsing; (2) **"PLAY FROM" stream buttons overlapped into unreadable garbage** whenever an addon (a real-world torrent addon in this case) returned dozens of long release-name labels — `DetailButton` never clipped/elided its text; now capped, clipped, elided; (3) **stale single-addon-name heading + duplicate "Found N streams" line** left over from the pre-S21 single-addon-browsing UI, removed — S21 already merges every addon's streams into one list; (4) **Jikan removed entirely** (see its own row); (5) **streams and search results are now ranked**, not shown in raw addon-response order: `rank_streams_for_playability` stable-sorts a `Direct` (actually playable) stream to the front so the primary "PLAY FROM" button always works on click; `rank_search_results_by_relevance` sorts merged search results by exact/starts-with/contains match against the query so the title actually searched for is what shows first | `client/ui/app.slint`, `client/ui/title-detail.slint`, `client/src/main.rs` |
...
| MyAnimeList forum discussions (`core::mal_forums`) | **retired and deleted by user request** (session 18) — client, credentials, fixtures/live tests, Home panel and the old unreachable inline Home copy were removed. No MAL forum/network/account behavior remains. (Jikan, the other MAL-adjacent surface — read-only ratings — was later removed too, session 23; see its own row) | `core/src/lib.rs`, `client/ui/home-dashboard.slint`, `client/ui/app.slint`, `client/src/main.rs` |
...
| Reachable interaction and compact-layout audit | **complete in sessions 21–22** — a new strict CI-style gate verifies all 58 exported callbacks, seven named routes, actionable TouchArea keyboard/focus/a11y semantics, async loading/error/empty/retry coverage, player/global-search shortcuts, placeholder destinations and compact minimums. The audit found and fixed five real gaps: Home modules now have truthful independent states, successful empty anime no longer appears stuck loading, global search is labelled, the app owns a 480×360 minimum with shrinking/scrolling content, and Home friend/discussion rows preserve the exact destination. Visible mutations now report success/failure instead of silently returning; fullscreen Hangout playback correctly promotes to window-wide cinema. A fresh macOS launch/screenshot then caught unsupported decorative glyphs that static checks cannot see; logo/search/loading/player/nav glyphs now use font-safe symbols and intentional letter tiles instead of rendering empty boxes | `scripts/check-interactions.sh`, `docs/qa/interaction-audit.md`, `client/ui/app.slint`, `client/ui/home-dashboard.slint`, `client/ui/shell-components.slint`, `client/src/main.rs` |
...
**This numbered list is historical (as of session 15) and stale — kept for the
narrative trail, not as the live candidate list.** Sessions 16-22 closed the
...
- Whether/when to formally open the repo to outside PRs is still undecided
  ...
  — see README).
- **ADR-0009 (account system) needs an Accept/reject decision.** It recommends
  an account be a self-custodied Ed25519 keypair with a 24-word recovery
  phrase (no password/email/OAuth — see the phase table above and the ADR
  itself for why). If accepted, real work follows: wiring `core::account`
  into `identity::load_or_create_device`, building the device-linking/QR
  ceremony, and the MLS group-membership fan-out on device add/remove. If
  rejected, say which alternative (password+email, magic link, OAuth) and
  why, so the ADR can be revised rather than silently reworked.
