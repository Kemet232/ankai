...
| Stremio competitive-parity roadmap | **planned in session 20** — benchmarked against the current official addon contract and desktop feature surface. The roadmap separates protocol completeness from product parity and defines acceptance gates for typed capability routing, every documented resource/stream form, Board/Discover, unified Library/Calendar/notifications, subtitles, torrent and packaged-source resolvers, binge playback, casting/local/offline media, addon discovery/trust, profiles/sync and no-install signed releases. Desktop parity is the first gate; mobile/TV follow after the shared core is proven | `docs/roadmaps/stremio-competitive-parity.md` |
